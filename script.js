function conversionTable(tagId, fahrenheit)
{
    var first = document.getElementById("Input1");
    fahrenheit = parseInt(first.value);
    ...
    if (!tab.querySelector('thead')) {
        ...
        tr.appendChild(th);
        th.appendChild(document.createTextNode("celcius"));
    }
    ...
    td.appendChild(document.createTextNode(fahrenheit));
    tr.appendChild(td);

    td = document.createElement("td");
    td.appendChild(document.createTextNode(c2f(fahrenheit)));
    tr.appendChild(td);
    
    function c2f(c) {return (c-32)*0.55555556;
}
}